...
const out=process.env.SPELL_CATALOG_OUTPUT || 'Recordings/SpellAnimationRevision/Planning';
...
if(edit){
 ...
 set('Targeting','C9','착탄 지점 고정. 시전 마법진 없음.\n1.5초 가속 낙하, 접촉 후 0.09초에 피해. 잔열 2회.');
 const guide=w.worksheets.getItem('Guide'),rows=guide.getUsedRange().values;
 for(let i=0;i<rows.length;i++){
  if(rows[i][0]==='버전')set('Guide',`B${i+1}`,'게임 0.13.2 운영 개정 2026-09-20. 9종, ID 0–5·7–9. 삭제된 ID 6은 재사용하지 않습니다.');
 ...
 w.recalculate();
 const exportDir=process.env.SPELL_CATALOG_EXPORT || 'outputs/spell-animation-20260920';await fs.mkdir(exportDir,{recursive:true});
