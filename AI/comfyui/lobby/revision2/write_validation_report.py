...
import json,hashlib
root=Path.cwd();out=root/'Recordings/LobbyRevision2/Device'
def read(name):return json.loads((out/name).read_text(encoding='utf-8'))
lobby=read('lobby-paired-summary.json');damage=read('damage-paired-summary.json');soak=read('final-soak.json');state=read('final-ready.json')
checks=read('final-checks.json');res=read('final-resolution-checks.json');audio=read('extended-checks.json')
apk=out/'KingdomIdle-LobbyQA.apk'
rows=[]
for title,data in [('로비',lobby),('데미지 표시 부하',damage)]:
 rows.append(f"| {title} | {data['normalCpuCorePercent']:.2f}% | {data['lowCpuCorePercent']:.2f}% | {data['cpuReductionPercent']:.1f}% | {data['normalFps']:.2f} / {data['lowFps']:.2f} |")
reduction=(1-damage['lowPresentation']['damageMicrosecondsPerFrame']/damage['normalPresentation']['damageMicrosecondsPerFrame'])*100
report=f"""# 로비 2차 리워크 검증 — 2026-09-12
...
각 모드 {lobby['secondsPerTrial']:.0f}초씩 일반→저사양→저사양→일반 ABBA 측정. Android `Process.getElapsedCpuTime` 증가량/벽시계 시간으로 프로세스 전체 CPU를 측정했다. 100%는 CPU 코어 하나를 계속 사용한 양이다. 프레임 대기 시간이 섞인 Unity Main Thread 시간으로 CPU 절약을 판단하지 않았다.

| 구간 | 일반 CPU | 저사양 CPU | CPU 감소 | FPS 일반 / 저사양 |
|---|---:|---:|---:|---:|
{chr(10).join(rows)}

- 로비 애니메이션 표식: {lobby['normalPresentation']['lobbyMicrosecondsPerFrame']:.2f} → {lobby['lowPresentation']['lobbyMicrosecondsPerFrame']:.2f} µs/frame.
- 데미지 연출 표식: {damage['normalPresentation']['damageMicrosecondsPerFrame']:.2f} → {damage['lowPresentation']['damageMicrosecondsPerFrame']:.2f} µs/frame, {reduction:.1f}% 감소.
- 120초 일반 로비 연속 실행: {soak['fps']:.2f} FPS, P99 {soak['frameMs']['p99']:.2f}ms, 50ms 초과 {soak['frameMs']['over50ms']}회, 샘플 내 managed allocation {soak['gc']['bytes']} bytes.
...
- 실기기 직접 입력 {sum(c['passed'] for c in checks)}/{len(checks)} 통과: 로그인/바깥 탭/Android Back, 언어 선택·현재 언어, 버튼 크기 복원, 배경 가림 회귀, 연출 정지/재개, 반복 조작, 인게임 설정 동기화, 완료 버튼.
- 화면 {len(res)}개: 720×1280, 1440×3088, 1536×2048, 1440×1600. 실제 `Screen.safeArea` 안의 제목·터치 영역·언어 팝업·설정 패널, 계정 버튼/Back 확인. 기기 크기/밀도는 검사 전 값으로 복구했다.
...
Lobby의 Moonwell Drift 1/2, InGame의 Quest to the Skyforge 1/2 및 Skybound Relic 1/2를 모두 순환 검사했다. {sum(c['passed'] for c in audio)}/{len(audio)} 음악·수치·상태 복구 검사 통과. 실제 AudioSource 진행 시간, Streaming 로드 방식, 출력 믹스 RMS 신호를 확인했다. 곡의 끝부분으로 이동해 다음 곡과 전체 순환도 확인했다. 물리 스피커 청취 판정은 포함하지 않는다.
...
- 최종 APK: {apk.stat().st_size:,} bytes ({apk.stat().st_size/1048576:.2f} MiB). 개발용 심볼 등을 포함한 BuildReport.totalSize와 APK 파일 크기는 다르다.
- SHA-256: `{hashlib.sha256(apk.read_bytes()).hexdigest()}`
...
"""
(root/'AI/comfyui/lobby/revision2/device-validation.md').write_text(report,encoding='utf-8')
print('Wrote device-validation.md')
