...
일반 라이트닝은 최초 `ThunderEffects` 5프레임·12fps 원화를 사용합니다. 2번째 프레임 타격 시점(약 0.167초)마다 다음 낙뢰를 이어 기본 3회, 4각성 4회, 8각성 이상 5회 시전합니다. 첫 낙뢰 이후에는 원래처럼 중심에서 0.8 이내에 분산하며, 각 낙뢰의 판정 반경은 0.55입니다. 수동 조준은 두 반경을 합친 전체 도달 범위 1.35를 표시합니다. 천벌 개화의 뇌운·단일 강타 연출은 별도로 유지합니다.
