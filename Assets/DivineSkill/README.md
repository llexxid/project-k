...
**완성 (2차, 2026-08-16) — 인게임 테스트 루프**
- **컬렉션북** — HUD "도감" 버튼 → `Popup_DivineCollection`: 카드 8종 그리드(등급/레벨/중복/장착 상태),
  상세(일러스트·설명·예상 수치), [장착]/[레벨업]. 이벤트 구동(Update 폴링 없음), 셀 인플레이스 갱신
- **신 스킬 뽑기** — 가챠 패널에 "신 스킬 뽑기" 탭 (Gold 10,000, 클라 롤 — 영웅 80/전설 15/신화 5%).
  중복 카드는 자동으로 레벨업 재료. **첫 카드 획득 시 시스템 자동 해금** (3-10 경로와 병행, 의도된 BM 동작)
- 인게임 테스트: 메인 화면 → 뽑기 탭에서 카드 획득 → 우하단 궁극기 버튼 등장 → 도감에서 장착 → 탭 시전

**아직 없는 것**
- 녹스의 '처치 골드 +100%', 컷인 중 전투 일시 정지(기획 3.4.1) 및 연출 스킵 옵션, 환생 계승 처리
- 나머지 7종 카드의 아트/전용 VFX (수치는 전부 동작 — 연출만 생략됨), 뽑기 서버 라우팅(현재 클라 롤)
