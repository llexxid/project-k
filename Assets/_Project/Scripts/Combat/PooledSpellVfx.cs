...
{
    /// <summary>
    /// 전투 스킬 VFX 프리팹에 붙는 수명/배치 컨트롤러 + 프리팹별 풀.
    ///
    ...
    public sealed class PooledSpellVfx : MonoBehaviour
    {
        ...
        // ── 풀 ──
        private static readonly Dictionary<GameObject, Stack<PooledSpellVfx>> _pools = new();
        ...
        private void Awake()
        {
            ...
            for (int i = 0; i < _renderers.Length; i++) _baseAlphas[i] = _renderers[i].color.a;
        ...
        public void Release()
        {
            ...
            {
                stack = new Stack<PooledSpellVfx>();
        ...
        /// </summary>
        public static PooledSpellVfx Spawn(GameObject prefab, Vector3 position,
                                              ...
                                              float scaleMultiplier = 1f)
        {
            ...
            var prefabDefaults = prefab.GetComponent<PooledSpellVfx>();

            if (inst == null)
            {
                var go = Instantiate(prefab, position, Quaternion.identity);
                inst = go.GetComponent<PooledSpellVfx>();
                if (inst == null) inst = go.AddComponent<PooledSpellVfx>();
