...
{
    /// <summary>
    /// Assets/_Project/Scripts/Stage_Catalog.xlsx를 StageDatabaseSO와 eStage enum으로 변환한다.
    /// 런타임에서는 엑셀을 읽지 않으며, 기획 데이터가 바뀔 때 이 메뉴를 다시 실행하면 된다.
    /// </summary>
    public static partial class StageDataGenerator
    {
        private const string ExcelRelativePath = "Assets/_Project/Scripts/Stage_Catalog.xlsx";
        private const string DatabaseAssetPath = "Assets/_Project/Resources/StageDatabaseSO.asset";
        ...
        public static void Generate()
        {
            try
            {
                ...
                if (PrepareMonsterEnum(workbook)) return;
                ...
                var catalog = ReadCatalog(workbook, stages, excelPath);
        ...
        private static void ReadStageMonsters(DataSet workbook, List<StageDraft> stages)
        {
            ...
            for (int rowIndex = 0; rowIndex < sheet.Rows.Count; rowIndex++)
            {
                ...
                int excelRow = rowIndex + 2;
                if (ReadInt(row, "Count", sheetName, excelRow) == 0) continue;
        ...
        private static List<BossFlowRecord> ReadBossFlows(DataSet workbook)
        {
            ...
            if (!workbook.Tables.Contains(sheetName)) return new List<BossFlowRecord>();
            DataTable sheet = GetRequiredSheet(workbook, sheetName);
        ...
        private static void WriteStageEnum(List<StageDraft> stages, string projectRoot)
        {
            ...
            source.AppendLine("// Stage_Catalog.xlsx에서 생성됩니다. 직접 수정하지 마세요.");
            source.AppendLine("// </auto-generated>");
        ...
        private sealed class StageDraft
        {
            ...
            public StageEncounterData Encounter;
            ...
            {
                var record = new StageDatabaseRecord(
                    Id,
                    FlowType,
                    EnvironmentId,
