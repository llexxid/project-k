...
public static class QuestCardCapture
{
    ...
    private static void AddRow(QuestDefinition definition, QuestCatalog catalog, Scene scene, Transform parent)
    {
        ...
        {
            // 고정 시안: 첫 행은 진행 중, 두 번째는 수령 가능, 세 번째는 수령 완료다.
            int index = parent.childCount - 1;
            var state = index == 1 ? QuestRowState.Claimable : index == 2 ? QuestRowState.Claimed : QuestRowState.InProgress;
            var snapshot = new QuestRowSnapshot(new QuestClaimToken(0, definition.QuestId, "preview"), definition.Category,
                definition.Title, definition.Description, definition.ObjectiveType, definition.TargetId, definition.RequiredCount,
                state == QuestRowState.InProgress ? definition.RequiredCount / 2 : definition.RequiredCount, state, false, 0,
                catalog.GetRewards(definition.RewardGroupId).Select(x => new QuestRewardSnapshot(x.Currency, x.Amount, x.IsDynamicGold)));
            row.SetQuest(snapshot, AssetDatabase.LoadAssetAtPath<UIViewCatalog>("Assets/UGUI/UIViewCatalog.asset"));
            return;
        }
